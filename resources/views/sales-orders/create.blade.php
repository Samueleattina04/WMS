<x-app-layout>
    <x-slot:title>Nuovo Ordine di Vendita</x-slot>
    <x-page-header title="Nuovo Ordine di Vendita">
        <x-slot:actions>
            <x-btn href="{{ route('sales-orders.index') }}" variant="secondary">Annulla</x-btn>
        </x-slot:actions>
    </x-page-header>

    <form method="POST" action="{{ route('sales-orders.store') }}" x-data="orderForm()">
        @csrf
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <template x-for="(item, index) in items" :key="index">
                        <div class="flex gap-3 mb-3 items-end">
                            ...
                            <div class="w-28">
                                ...
                                <input type="number" :name="`items[${index}][quantity_requested]`" min="1" x-model="item.qty" class="block w-full rounded-md border-gray-300 shadow-sm text-sm" required>
                            </div>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 class="text-base font-semibold text-gray-900 mb-4">Dettagli</h3>
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Cliente</label>
                            <select name="customer_id" class="block w-full rounded-md border-gray-300 shadow-sm text-sm">
    ...
    <script>
        function orderForm() {
            return { items: [{qty:1}], addItem() { this.items.push({qty:1}); }, removeItem(i) { if(this.items.length>1) this.items.splice(i,1); } }
        }
    </script>
</x-app-layout>
