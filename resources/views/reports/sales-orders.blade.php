<x-app-layout>
    <x-slot:title>Report Ordini di Vendita</x-slot>
    <x-page-header title="Report Ordini di Vendita"/>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead><tr class="bg-gray-50">
                    ...
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Articoli</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Data</th>
                </tr></thead>
                <tbody class="divide-y divide-gray-200">
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm font-medium text-gray-900"><a href="{{ route('sales-orders.show', $order) }}" class="text-indigo-600 hover:underline">{{ $order->order_number ?? '#'.$order->id }}</a></td>
                            ...
                            <td class="px-6 py-4 text-right text-sm">{{ $order->items->count() }}</td>
